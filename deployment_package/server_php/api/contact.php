...
switch($method) {
    ...
    case 'PUT':
        ...
        try {
            if ($id) {
                // If is_read column exists it will update, if not it catches the exception silently
                $stmt = $db->prepare("UPDATE contacts SET is_read = 1 WHERE id = :id");
                $stmt->bindParam(':id', $id);
            } else {
                $stmt = $db->prepare("UPDATE contacts SET is_read = 1 WHERE is_read = 0");
            }
            $stmt->execute();
            sendResponse("success", "Contacts marked as read", []);
        } catch (Exception $e) {
            sendResponse("error", "Database error", [], 500);
        }
        break;
